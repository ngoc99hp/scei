  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border-b border-amber-100">
      ...
      <button
        type="button"
        onClick={onClose}
        className="ml-auto text-gray-400 hover:text-gray-600 text-xs px-1"
      >
        ✕
      </button>
    </div>
...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    extensions: [
      StarterKit.configure({
        ...
        bulletList: {
          HTMLAttributes: { class: "list-disc pl-5 space-y-1 my-3" },
        },
        orderedList: {
          HTMLAttributes: { class: "list-decimal pl-5 space-y-1 my-3" },
        },
        blockquote: {
          HTMLAttributes: {
            class:
              "border-l-[3px] border-gray-300 pl-4 text-gray-500 italic my-4",
          },
        },
        code: {
          HTMLAttributes: {
            class:
              "bg-gray-100 text-rose-600 rounded px-1.5 py-0.5 text-[0.85em] font-mono",
          },
        },
        codeBlock: {
          HTMLAttributes: {
            class:
              "bg-gray-950 text-gray-100 rounded-lg p-4 text-sm font-mono overflow-x-auto my-4 leading-relaxed",
          },
        },
    ...
    editorProps: {
      attributes: {
        class:
          "relative focus:outline-none text-gray-800 leading-[1.8] text-[15px] [&_h1]:text-[1.6rem] [&_h1]:font-bold [&_h1]:text-gray-900 [&_h1]:mt-8 [&_h1]:mb-3 [&_h1]:tracking-tight [&_h1]:leading-tight [&_h2]:text-xl [&_h2]:font-bold [&_h2]:text-gray-900 [&_h2]:mt-7 [&_h2]:mb-2.5 [&_h2]:leading-tight [&_h3]:text-[1.05rem] [&_h3]:font-semibold [&_h3]:text-gray-800 [&_h3]:mt-5 [&_h3]:mb-2 [&_p]:my-2.5 [&_p]:text-gray-700 [&_strong]:font-semibold [&_strong]:text-gray-900 [&_a]:text-blue-600 [&_a]:underline [&_a]:underline-offset-2",
      },
    },
    onUpdate({ editor }) {
  ...
  if (!editor) {
    return (
      <div
        className="border border-gray-200 rounded-xl animate-pulse bg-gray-50"
  ...
  return (
    <div
      className={[
        ...
        fullscreen
          ? "fixed inset-3 z-9999 rounded-2xl shadow-2xl border-gray-300"
          : "",
      ].join(" ")}
    >
      {/* ── Toolbar ── */}
      <div className="flex flex-wrap items-center gap-0.5 px-2.5 py-1.5 bg-gray-50/90 border-b border-gray-100 sticky top-0 z-10">
        ...
        <div className="ml-auto">
          <Btn
            ...
            label={fullscreen ? "Thu nhỏ (Esc)" : "Toàn màn hình"}
          >
      ...
      {/* ── Context bars ── */}
      {showFind && (
        <FindReplaceBar editor={editor} onClose={() => setShowFind(false)} />
      )}
      {showLink && (
        <LinkBar editor={editor} onClose={() => setShowLink(false)} />
      )}
