      ...
      <Section className="relative py-24 overflow-hidden bg-background">
        <Container className="relative z-10 text-center max-w-4xl mx-auto">
          ...
          <p className="mt-8 text-xl text-muted-foreground leading-relaxed">
            Được dẫn dắt bởi những chuyên gia hàng đầu, luôn sẵn sàng chia sẻ kiến thức để giúp bạn kiến tạo giá trị.
          </p>
        </Container>
      </Section>
        ...
        <Container>
          <div className="relative rounded-[2.5rem] bg-primary p-12 lg:px-20 lg:py-16 text-white flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="max-w-xl text-center lg:text-left">
              ...
              <p className="text-white/80 text-lg">Bạn muốn đóng góp cho sự phát triển của hệ sinh thái khởi nghiệp? Hãy đồng hành cùng chúng tôi.</p>
            </div>
            <Link href="/contact">
...
// Server Component — không dùng onClick hay bất kỳ event handler nào
function MentorCard({ mentor }) {
  return (
    <Card className="group overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all duration-300">
      <div className="relative h-64 overflow-hidden">
        {mentor.avatar
          ? <img src={mentor.avatar} alt={mentor.name} className="h-full w-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500 scale-105 group-hover:scale-110" />
          : <div className="h-full w-full bg-primary/10 flex items-center justify-center text-6xl font-bold text-primary">{mentor.name[0]}</div>
        }
