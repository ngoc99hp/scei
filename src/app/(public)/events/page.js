...
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { Calendar as CalendarIcon, MapPin, Clock, Users, ChevronRight } from "lucide-react"

export const revalidate = 1800

const typeLabel = { WORKSHOP:"Workshop", PITCHING:"Pitching", NETWORKING:"Networking", SEMINAR:"Seminar", CONFERENCE:"Conference", OTHER:"Khác" }
const statusDot  = { OPEN:"bg-green-500", FULL:"bg-red-500", ONGOING:"bg-blue-500", COMPLETED:"bg-gray-400", CANCELLED:"bg-gray-400", DRAFT:"bg-yellow-500" }
const statusText = { OPEN:"Đang mở đăng ký", FULL:"Đã đủ chỗ", ONGOING:"Đang diễn ra", COMPLETED:"Đã kết thúc", CANCELLED:"Đã hủy", DRAFT:"Sắp mở" }
    ...
    <>
        ...
        <Section className="py-16">
          <Container>
            <h2 className="text-2xl font-bold mb-8 flex items-center gap-2">
              <span className="h-8 w-1 bg-primary rounded-full" /> Sự kiện tiêu điểm
            </h2>
            <Card className="overflow-hidden border-none shadow-2xl bg-slate-900 group">
              <div className="flex flex-col lg:flex-row">
                <div className="relative h-64 lg:h-auto lg:w-3/5 overflow-hidden">
                  {featured.cover_image
                    ? <img src={featured.cover_image} alt={featured.title} className="h-full w-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-700" />
                    : <div className="h-full w-full bg-primary/20 min-h-64" />
                  }
                  <div className="absolute inset-0 bg-gradient-to-r from-slate-950/80 to-transparent" />
                </div>
                <div className="p-8 lg:p-12 flex flex-col justify-center lg:w-2/5 text-white">
                  <Badge className="w-fit bg-primary hover:bg-primary text-white border-none mb-6">{typeLabel[featured.type] || featured.type}</Badge>
                  <h3 className="text-3xl lg:text-4xl font-bold leading-tight">{featured.title}</h3>
                  <p className="mt-6 text-slate-300 text-lg">{featured.short_desc}</p>
                  <ul className="mt-8 space-y-4 text-sm text-slate-400">
                    <li className="flex items-center gap-3">
                      <CalendarIcon className="text-primary h-5 w-5" />
                      {new Date(featured.start_date).toLocaleDateString("vi-VN", { day: "2-digit", month: "long", year: "numeric" })}
                    </li>
                    <li className="flex items-center gap-3">
                  ...
                  <div className="mt-10">
                    <Link href={`/events/${featured.slug}`}>
                      <Button className="w-full sm:w-auto px-10 rounded-full font-bold">
                        Đăng ký ngay <ChevronRight className="ml-2 h-4 w-4" />
                      </Button>
                    </Link>
                  </div>
      ...
      {upcoming.length > 0 && (
        <Section className="py-16 bg-muted/10">
          <Container>
            <h2 className="text-2xl font-bold mb-8 flex items-center gap-2">
              <span className="h-8 w-1 bg-primary rounded-full" /> Sắp diễn ra
            </h2>
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {events.length === 0 && (
        <Section className="py-32">
          <Container className="text-center">
            <Users className="mx-auto h-12 w-12 text-muted-foreground opacity-20 mb-4" />
            <p className="text-xl text-muted-foreground">Chưa có sự kiện nào được công bố.</p>
          </Container>
        </Section>
      )}
    </>
...
function EventCard({ event, past = false }) {
  ...
  return (
    <Card className={`group overflow-hidden border border-border hover:shadow-xl transition-all duration-300 flex flex-col ${past ? "opacity-70" : ""}`}>
      <div className="relative aspect-video overflow-hidden">
        {event.cover_image
          ? <img src={event.cover_image} alt={event.title} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" />
          : <div className="h-full w-full bg-muted flex items-center justify-center text-sm text-muted-foreground">Không có ảnh</div>
        }
        <div className="absolute top-4 left-4">
          <Badge className="bg-background/95 text-foreground border-none shadow-sm text-xs">{typeLabel[event.type] || event.type}</Badge>
        </div>
        <div className="absolute top-4 right-4 flex items-center gap-1.5">
          <span className={`h-2 w-2 rounded-full ${statusDot[event.status] || "bg-gray-400"}`} />
          <span className="text-xs font-medium text-white drop-shadow">{statusText[event.status] || event.status}</span>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex items-center gap-2 text-primary font-bold text-sm mb-3">
          ...
          {startDate.toLocaleDateString("vi-VN", { day: "2-digit", month: "short", year: "numeric" })}
        </div>
        <h3 className="text-xl font-bold leading-tight group-hover:text-primary transition-colors line-clamp-2">{event.title}</h3>
        <p className="text-sm text-muted-foreground mt-2 line-clamp-2">{event.short_desc}</p>
        <div className="mt-4 space-y-2 text-sm text-muted-foreground border-t border-border pt-4">
          <div className="flex items-center gap-2">
            <Clock size={14} />
          ...
          {spotsLeft !== null && spotsLeft > 0 && (
            <div className="flex items-center gap-2 text-green-600">
              <Users size={14} /> Còn {spotsLeft} suất
            </div>
          )}
        </div>
        <div className="mt-auto pt-4">
          <Link href={`/events/${event.slug}`}>
            <Button variant="outline" className="w-full rounded-full text-sm font-bold gap-2 group/btn">
              {past ? "Xem lại" : "Chi tiết & Đăng ký"}
